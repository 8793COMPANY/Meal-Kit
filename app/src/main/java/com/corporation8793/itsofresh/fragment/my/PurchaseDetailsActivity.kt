...
//        val purchase_progress = view.findViewById<RelativeLayout>(R.id.purchase_progress)
        ...
        adapter.setOnItemClickListener(object : PurchaseAdapter.OnItemClickListener{
            ...
            if (item.isNotEmpty()) {
                datas.apply {
                    item.forEach {
                        if (!it.status.equals("pending") || !it.status.equals("failed") ) {
                            var img_pos = 0
                            ...
                            if (it.meta_data.filter { orderMeta -> orderMeta.key == "is_parcel" }.size != 0)
                                address_type =
                                    it.meta_data.filter { orderMeta -> orderMeta.key == "is_parcel" }
                                        .get(0).value.toString()
//                        Log.e("in!",it.meta_data.filter { orderMeta ->  orderMeta.key == "is_parcel"}.get(0).value.toString())
                            var paid_point =
                                it.meta_data.filter { orderMeta -> orderMeta.key == "paid_point" }
                            ...
                            add(
                                PurchaseItem(
                                    ...
                                    "0",
                                    it.id.toString(),
                                    it.line_items.get(0).product_id,
                                    it.date_created!!.replace("T", " "),
                                    it.meta_data.get(0).value.toString(),
                                    it.line_items.get(0).name.toString(),
                                    it.line_items.get(0).total,
                                    it.line_items.get(0).quantity,
                                    status,
                                    it.billing.address_1 + "\n" + it.billing.address_2,
                                    paid_point,
                                    payment_way
                                )
                            )
                            ...
                            GlobalScope.launch(Dispatchers.Default) {
                                var img =
                                    MainApplication.instance.boardRepository.retrieveOneProduct(
                                        it.line_items.get(0).product_id
                                    ).second!!.images.get(0).src
